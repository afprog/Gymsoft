 ...
 * 
 * @see co.edu.uniminuto.arqsoft.daos.Pago
...
public class PagoDAO implements IPagoDAO {
	...
	public void save(Pago entity) {
		EntityManagerHelper.log("saving Pago instance", Level.INFO, null);
		try {
	...
	public void delete(Pago entity) {
		EntityManagerHelper.log("deleting Pago instance", Level.INFO, null);
		try {
			entity = getEntityManager().getReference(Pago.class,
					entity.getIdPago());
	...
	public Pago update(Pago entity) {
		EntityManagerHelper.log("updating Pago instance", Level.INFO, null);
		try {
			Pago result = getEntityManager().merge(entity);
	...
	public Pago findById(Integer id) {
		EntityManagerHelper.log("finding Pago instance with id: " + id,
				Level.INFO, null);
		try {
			Pago instance = getEntityManager().find(Pago.class, id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List<Pago> findByProperty(String propertyName, final Object value,
			final int... rowStartIdxAndCount) {
		EntityManagerHelper.log("finding Pago instance with property: "
				+ propertyName + ", value: " + value, Level.INFO, null);
		try {
			final String queryString = "select model from Pago model where model."
	...
	public List<Pago> findByValor(Object valor, int... rowStartIdxAndCount) {
		return findByProperty(VALOR, valor, rowStartIdxAndCount);
	}
	 ...
	 */
	@SuppressWarnings("unchecked")
	public List<Pago> findAll(final int... rowStartIdxAndCount) {
		EntityManagerHelper.log("finding all Pago instances", Level.INFO, null);
		try {
			final String queryString = "select model from Pago model";
			Query query = getEntityManager().createQuery(queryString);
			if (rowStartIdxAndCount != null && rowStartIdxAndCount.length > 0) {
				int rowStartIdx = Math.max(0, rowStartIdxAndCount[0]);
				if (rowStartIdx > 0) {
