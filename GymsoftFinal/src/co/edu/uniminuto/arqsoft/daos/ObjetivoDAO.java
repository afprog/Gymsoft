 ...
 * Objetivo entities. Transaction control of the save(), update() and delete()
 * operations must be handled externally by senders of these methods or must be
 * manually added to each of these methods for data to be persisted to the JPA
 * datastore.
 * 
 * @see co.edu.uniminuto.arqsoft.daos.Objetivo
...
public class ObjetivoDAO implements IObjetivoDAO {
	...
	public void save(Objetivo entity) {
		EntityManagerHelper.log("saving Objetivo instance", Level.INFO, null);
		try {
	...
	public void delete(Objetivo entity) {
		EntityManagerHelper.log("deleting Objetivo instance", Level.INFO, null);
		try {
			entity = getEntityManager().getReference(Objetivo.class,
					entity.getIdObjetivo());
	 ...
	 * <pre>
	 * EntityManagerHelper.beginTransaction();
	 * entity = ObjetivoDAO.update(entity);
	 * EntityManagerHelper.commit();
	 * </pre>
	 * 
	 * @param entity
	...
	public Objetivo update(Objetivo entity) {
		EntityManagerHelper.log("updating Objetivo instance", Level.INFO, null);
		try {
			Objetivo result = getEntityManager().merge(entity);
	...
	public Objetivo findById(Integer id) {
		EntityManagerHelper.log("finding Objetivo instance with id: " + id,
				Level.INFO, null);
		try {
			Objetivo instance = getEntityManager().find(Objetivo.class, id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List<Objetivo> findByProperty(String propertyName,
			final Object value, final int... rowStartIdxAndCount) {
		EntityManagerHelper.log("finding Objetivo instance with property: "
				+ propertyName + ", value: " + value, Level.INFO, null);
		try {
			final String queryString = "select model from Objetivo model where model."
	 ...
	 */
	@SuppressWarnings("unchecked")
	public List<Objetivo> findAll(final int... rowStartIdxAndCount) {
		EntityManagerHelper.log("finding all Objetivo instances", Level.INFO,
				null);
		try {
			final String queryString = "select model from Objetivo model";
			Query query = getEntityManager().createQuery(queryString);
			if (rowStartIdxAndCount != null && rowStartIdxAndCount.length > 0) {
				int rowStartIdx = Math.max(0, rowStartIdxAndCount[0]);
				if (rowStartIdx > 0) {
